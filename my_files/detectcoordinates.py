...
from utilss.dataloaders import IMG_FORMATS, VID_FORMATS, LoadImages, LoadScreenshots, LoadStreams
from utilss.general import (LOGGER, Profile, check_file, check_img_size, check_imshow, check_requirements, colorstr, cv2,
                           increment_path, non_max_suppression, print_args, scale_boxes, strip_optimizer, xyxy2xywh)
from utilss.plots import Annotator, colors, save_one_box
...
#     #adding headers
#     f.write('x1 \ty1 \tx2 \ty2 \tw \th \tconf \tcls \tname \ttime')
#     f.write('\n')
#     f.close()
...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        try:
            if(x1, y1, x2, y2 != xyxy[0], xyxy[1], xyxy[2], xyxy[3]):
                with open('results.txt', 'a') as f:
                    for *xyxy, conf, cls in reversed(det):
                        ...
                        f.write(f'{x1} {y1} {x2} {y2} {w} {h} {conf} {cls} {names[int(cls)]} {datetime.datetime.now()}')
                        f.write('\n')
            ...
            with open('indexes.txt', 'w') as f:
                for *xyxy, conf, cls in reversed(det):
                    ...
                    x_index, y_index = get_matrix_index(x1, y1, x2, y2, image_width, image_height, num_rows, num_cols)
    ...
    #makinf function that converts xyxy to xywh
    # def xyxy2xywh(x):
    ...
    #calling the function and saving results in a txt file
    # with open('results.csv', 'a') as f:
    #     x1, y1, x2, y2 = xyxy[0], xyxy[1], xyxy[2], xyxy[3]
    #     w = x2 - x1
    #     h = y2 - y1

    #     f.write(f'{x1} {y1} {w} {h} {conf} {cls} {names[int(cls)]}')
    #     f.write('\n')
    #     f.close()
